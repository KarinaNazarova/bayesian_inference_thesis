...
# Load real data (with imputed missing values)
data = pd.read_excel('../imputed_data_combined_T1.xlsx', index_col=None, header=0)
data = data.rename(columns={'cdi_label': 'outcome'})

numeric_features = [
    'tdeescri', 'tdearitm', 'tdeleit', 'tdetotal', 'ls_self', 'ls_comparative_self', 
    'ls_non_violence', 'ls_family', 'ls_friendship', 'ls_school', 'panas_positive', 
    'panas_negative', 'cdi_social_withdrawal', 'cdi_anhedonia_asthenia', 
    'cdi_incompetence_maladjustment', 'cdi_negative_self_esteem', 
    'cdi_sleep_appetite_disturbances'
]
...
for model_type in models_to_test:
    ...
    active_features, inactive_features = identify_active_and_inactive_features(data, beta_samples)
