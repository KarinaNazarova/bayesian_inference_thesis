...
parameters {
  ...
  real<lower=0> tau;       // scale parameter for Laplace distribution. Smaller T => stronger shrinkage, as peak is tighter 
}
model {
  // Priors
  tau ~ normal(0, 1);
...
cauchy_sparse_logistic_regression_model = """
data {
  int<lower=1> N;          // number of observations
  int<lower=0> K;          // number of features
  matrix[N, K] X;          // predictor matrix
...
model {
  // Priors
  tau ~ normal(0, 1);
  beta ~ cauchy(0, tau); // Cauchy prior for coefficients
  alpha ~ normal(0, 10);
...
horseshoe_sparse_logistic_regression_model = """
data {
  int<lower=1> N;          // number of observations
  int<lower=0> K;          // number of features
  matrix[N, K] X;          // predictor matrix
...
parameters {
  ...
  real<lower=0> tau;       // global shrinkage parameter for horseshoe prior T (for all coefficients in the model simultaneously)
  vector<lower=0>[K] lambda; // local shrinkage parameters for each coefficient (for each specific coefficient. Small => stronger shrinkage)
}
...
model {
    ...
    beta ~ normal(0, lambda_hat); // Horseshoe prior for coefficients
...
horseshoe_sparse_logistic_regression_model_regularised = """
data {
    int <lower=0> N; // number of observations
    int <lower=0> K; // number of predictors
    matrix [N, K] X; // inputs
    ...
    real <lower=1> nu_global; // degrees of freedom for the tau student half-t priors (larger -> approximates normal)
    real <lower=0> scale_global; // scale for the half -t prior for tau

    real <lower=1> nu_local; // degrees of freedom for the lambda student half-t priors  (larger -> approximates normal)
...
parameters {
    real alpha;
    vector [K] z;     // latent variables that define coefficients 
    real <lower=0> tau; // global shrinkage parameter
    vector <lower=0>[K] lambda; // local shrinkage parameter
    real <lower=0> caux;   // used to calculate slab scale c 
}
transformed parameters {
...
def get_model_code(model_type):
    if model_type == SparseModelType.NORMAL:
        ...
        return laplace_sparse_logistic_regression_model
    elif model_type == SparseModelType.CAUCHY:
        return cauchy_sparse_logistic_regression_model
    elif model_type == SparseModelType.HORSESHOE:
        return horseshoe_sparse_logistic_regression_model
        ...
        return horseshoe_sparse_logistic_regression_model_regularised
...
def fit_model(data, model_type, chains=4, samples=1000):
    ...
    if model_type == SparseModelType.HORSESHOE_REGULARISED:
        model_data = {
          ...
          'X': X,
          'y': y,
          'scale_icept': 10,  # (paper) prior std for the intercept: larger => intercept can take more values: https://github.com/junpenglao/Planet_Sakaar_Data_Science/blob/4366de036cc608c942fdebb930e96f2cc8b83d71/WIP/Evaluating%20Variational%20Inference/Figure_7_Horseshoe.R
          'scale_global': (5 / (X.shape[1] - 5)) * (2 / np.sqrt(X.shape[0])), #(formula: (1/(d-1)) * (2/np.sqrt(n)), d-columns, n-rows) smaller value => more shrinkage towards 0
